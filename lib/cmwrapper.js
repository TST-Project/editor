(function(CodeMirror) {
    ...
    CodeMirror.defineMode('mixed', function(config) {
        return CodeMirror.multiplexingMode(
            ...
            {open: '{', close: '}',
                mode: CodeMirror.getMode(config, 'entity'),
                delimStyle: 'delimit'}
        );
    });
    ...
    const hintEntity = (cm,options) => {
        ...
        const newlist = list.filter(el => el.displayText.startsWith(word));
        return {list: newlist, from: from, to: to};
    };

    CodeMirror.registerHelper('hint','entity',hintEntity);
...
window.cmWrapper = (function() {
    ...
    const init = (textarea) => {
        ...
        const completeEntity = (cm) => {
            ...
            window.setTimeout(function() {
                if (!cm.state.completionActive)
                    cm.showHint({hint: CodeMirror.hint.entity, from: from});
            },100);
            return CodeMirror.Pass;
        };
    ...
    const getSchema = function(s) {
        const schemae = {
            transcription: ['milestone','lb','pb','add','choice','del','seg','subst','supplied','surplus','space','unclear','gap', 'damage','supplied','g'], 
        ...
        const tags = {
            ...
            seg: {
                attrs: {
                    'function': ['blessing','colophon','commenting-note','completion-statement','chapter-heading','correction','dedication','documenting-note','end-title','explicit','gloss','heading','incipit','intertitle','invocation','ownership-statement','postface','preface','register','rubric','running-title','satellite-stanza','shelfmark','stamp','title','table-of-contents','total-stanzas','total-chapters','verse-beginning','verse-numbering'],
                    'rend': ['grantha']
                },
